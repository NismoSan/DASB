...
function startBlackjack(sender, isWhisper) {
  if (S.bjGroupSession) {
    var seats = S.bjGroupSession.seats;
...
function bjDealHand(session) {
  ...
  setTimeout(function () {
    if (w) {
      ...
      if (S.sendWhisper) {
        var chunks = sm.splitMessage('BJ Hand: ' + bjFormatHand(session.playerHand), S.WHISPER_MAX);
...
function startBjLobby(sender, args) {
  ...
  var existing = S.activeGames.get(sender.toLowerCase());
  if (existing) {
...
function joinBjLobby(sender) {
  ...
  for (var i = 0; i < S.bjGroupSession.seats.length; i++) {
  ...
  var count = S.bjGroupSession.seats.length;
  var max = S.bjGroupSession.maxSeats;
...
function bjGroupStartGame() {
  ...
  var names = S.bjGroupSession.seats.map(function (s) {
    return s.name;
  }).join(', ');
...
function showBjStatus(sender, isWhisper) {
  var session = S.activeGames.get(sender.toLowerCase());
  if (session && session.gameType === 'blackjack') {
...
module.exports = {
  ...
  handleBlackjackAction: handleBlackjackAction,
  startBjLobby: startBjLobby,
  joinBjLobby: joinBjLobby,
  forceStartBj: forceStartBj,
  handleGroupBjMessage: handleGroupBjMessage,
  bjGroupHandleBet: bjGroupHandleBet,
  leaveBj: leaveBj,
  stopBjGame: stopBjGame,
  showBjStatus: showBjStatus,
