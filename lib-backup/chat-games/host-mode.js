...
function hostNextRound() {
  ...
  if (S.hostSession.currentRound > S.hostSession.totalRounds) {
    var finishedHost = S.hostSession.hostPlayer;
    var finishedType = S.hostSession.gameType;
  ...
  var roundLabel = 'Round ' + S.hostSession.currentRound + '/' + S.hostSession.totalRounds;
...
function hostSetQuestion(roundLabel, question, answer, hint) {
  ...
  S.hostSession.questionActive = true;
  var prefix = S.hostSession.gameType === 'scramble' ? 'Unscramble' : S.hostSession.gameType === 'riddle' ? 'Riddle' : 'Q';
...
function processNextAnswer() {
  ...
  var entry = S.hostAnswerQueue.shift();
  var gameType = S.hostSession.gameType;
...
function processNumberGuessEntry(entry) {
  ...
  var target = S.hostSession.currentQuestion.targetNumber;
  if (num === target) {
    S.hostAnswerQueue = [];
    S.hostAnswerProcessing = false;
...
function processScrambleEntry(entry) {
  ...
  var correctAnswer = S.hostSession.currentQuestion.answer.toLowerCase();
...
function processHangmanEntry(entry) {
  ...
  var q = S.hostSession.currentQuestion;
  ...
  if (input.length > 1) {
    ...
    if (q.wrongCount >= q.maxWrong) {
      ...
      S.hostAnswerQueue = [];
      S.hostAnswerProcessing = false;
      S.hostSession.roundTimer = setTimeout(function () {
        hostNextRound();
      }, S.HOST_DELAY_BETWEEN);
      return;
    }
...
function processTriviaRiddleEntry(entry) {
  ...
  var answer = S.hostSession.currentQuestion.answer;
  var playerText = entry.message.trim().toLowerCase();
...
function hostRoundWon(winner) {
  ...
  var entry = S.hostSession.leaderboard.get(key) || {
    name: winner,
    points: 0
  };
  entry.points++;
  ...
  var answer = S.hostSession.currentQuestion ? S.hostSession.currentQuestion.answer : '???';
...
function hostShowFinalLeaderboard() {
  if (!S.hostSession) return;
  var board = [];
  S.hostSession.leaderboard.forEach(function (entry) {
    board.push(entry);
  });
...
module.exports = {
  ...
  handleHostStop: handleHostStop,
  handleHostSkip: handleHostSkip,
  handleHostAnswer: handleHostAnswer,
