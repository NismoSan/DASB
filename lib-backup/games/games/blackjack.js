...
exports.startBlackjack = startBlackjack;
exports.handleBlackjackAction = handleBlackjackAction;
exports.startBjLobby = startBjLobby;
exports.joinBjLobby = joinBjLobby;
exports.forceStartBj = forceStartBj;
exports.bjGroupHandleBet = bjGroupHandleBet;
exports.handleGroupBjMessage = handleGroupBjMessage;
exports.leaveBj = leaveBj;
exports.stopBjGame = stopBjGame;
exports.showBjStatus = showBjStatus;
exports.getBjStatus = getBjStatus;
...
const BJ_MILESTONES = [100000, 250000, 500000, 1000000, 2500000, 5000000];
// -- Dealer Flavor Text --
const BJ_DEAL_COMMENTS = [
...
const BJ_DEALER_REVEAL = [
    '{=cDealer flips... ',
    '{=cAnd the hole card is... ',
    '{=cMoment of truth... ',
    '{=cDealer reveals... '
];
const BJ_HIT_REACTIONS_GOOD = [
    '{=dSolid card.',
    '{=dNot bad at all.',
    '{=dStill in it!'
];
const BJ_HIT_REACTIONS_RISKY = [
    '{=oGetting up there...',
    '{=oLiving dangerously!',
    '{=oBrave move.'
];
...
const BJ_HOT_STATUS = ['{=o[HOT] ', '{=o[FIRE] ', '{=o[STREAK] '];
const BJ_COLD_STATUS = ['{=b[COLD] ', '{=b[ICE] ', '{=b[ROUGH] '];
const BJ_STREAK_BROKEN = ['{=rStreak broken at ', '{=rRun ends at '];
const BJ_LOSS_STREAK = [
...
function bjGetStatusTag(session) {
    if (session.winStreak >= 3)
        return bjPickRandom(BJ_HOT_STATUS);
    if (session.lossStreak >= 3)
        return bjPickRandom(BJ_COLD_STATUS);
    return '';
}
function bjCheckMilestone(session, oldChips) {
    ...
    for (let i = 0; i < BJ_MILESTONES.length; i++) {
        ...
        if (oldChips < m && newChips >= m) {
            ...
            sm.sendGameResponse(session.playerName, '{=wMILESTONE! You hit ' + label + ' chips!', session.isWhisper);
        }
    }
...
function bjApplyStreakBonus(session, baseWinnings) {
    ...
    if (mult > 1.0) {
        ...
        sm.sendGameResponse(session.playerName, bjGetStreakLabel(session.winStreak) + ' +' + bonus, session.isWhisper);
...
function bjSaveChips(playerName, chips) {
    ...
    if (!stats.byGame)
        stats.byGame = {};
    if (!stats.byGame.blackjack)
        stats.byGame.blackjack = {
            wins: 0,
            played: 0
        };
...
function startBlackjack(sender, isWhisper) {
    if (state_1.default.bjGroupSession) {
        const seats = state_1.default.bjGroupSession.seats;
    ...
    state_1.default.activeGames.set(sender.toLowerCase(), session);
    if (state_1.default.ioRef) {
        ...
        state_1.default.ioRef.emit('chatgames:active', sm.getActiveGames());
    }
...
function bjPromptBet(session) {
    ...
    session.actionTimer = setTimeout(function () {
        if (!state_1.default.activeGames.has(session.playerName.toLowerCase()))
            return;
    ...
    setTimeout(function () {
        if (w) {
            ...
            if (state_1.default.sendWhisper) {
                const chunks = sm.splitMessage('BJ Hand: ' + bjFormatHand(session.playerHand), state_1.default.WHISPER_MAX);
                chunks.forEach(function (chunk, ci) {
                    setTimeout(function () {
                        state_1.default.sendWhisper(name, chunk);
                    }, ci * 500);
                });
            }
...
function bjPromptAction(session) {
    ...
    if (session.canDouble && !session.playingSplit)
        options += '/double';
    if (session.canSplit && !session.playingSplit && !session.splitHand)
        options += '/split';
    const tag = bjGetStatusTag(session);
...
function startBjLobby(sender, args) {
    ...
    const existing = state_1.default.activeGames.get(sender.toLowerCase());
    if (existing) {
    ...
    sm.setCooldown(sender);
    state_1.default.bjGroupSession = {
        hostPlayer: sender,
        phase: 'lobby',
        seats: [{
                name: sender,
...
function joinBjLobby(sender) {
    ...
    if (state_1.default.bjGroupSession.phase !== 'lobby') {
        sm.sendGameResponse(sender, 'Game already in progress.', false);
        return;
    }
    for (let i = 0; i < state_1.default.bjGroupSession.seats.length; i++) {
        if (state_1.default.bjGroupSession.seats[i].name.toLowerCase() === sender.toLowerCase()) {
    ...
    state_1.default.bjGroupSession.seats.push({
        name: sender,
        chips: BJ_STARTING_CHIPS,
        hand: [],
        splitHand: null,
    ...
    const count = state_1.default.bjGroupSession.seats.length;
    const max = state_1.default.bjGroupSession.maxSeats;
...
function forceStartBj(sender) {
    ...
    if (state_1.default.bjGroupSession.hostPlayer.toLowerCase() !== sender.toLowerCase()) {
    ...
    if (state_1.default.bjGroupSession.seats.length < 2) {
...
function bjGroupLobbyTimeout() {
    if (!state_1.default.bjGroupSession || state_1.default.bjGroupSession.phase !== 'lobby')
        return;
    if (state_1.default.bjGroupSession.seats.length >= 2) {
        sm.sendGameResponse(state_1.default.bjGroupSession.hostPlayer, 'Lobby timer up. Dealing!', false);
        bjGroupStartGame();
    }
    else {
        sm.sendGameResponse(state_1.default.bjGroupSession.hostPlayer, 'Not enough players. Cancelled.', false);
        bjGroupCleanup();
    }
}
function bjGroupStartGame() {
    if (!state_1.default.bjGroupSession)
        return;
    if (state_1.default.bjGroupSession.lobbyTimer) {
        clearTimeout(state_1.default.bjGroupSession.lobbyTimer);
        state_1.default.bjGroupSession.lobbyTimer = null;
    }
    const names = state_1.default.bjGroupSession.seats.map(function (s) {
        return s.name;
    }).join(', ');
    sm.sendGameResponse(state_1.default.bjGroupSession.hostPlayer, 'Blackjack starting! ' + names, false);
    sm.emitActivity(state_1.default.bjGroupSession.hostPlayer, 'blackjack', 'group game started');
    setTimeout(function () {
        bjGroupDealHand();
    }, 1500);
}
function bjGroupDealHand() {
    const gs = state_1.default.bjGroupSession;
    if (!gs)
        return;
    gs.handNumber++;
...
function handleGroupBjMessage(sender, message) {
    ...
    if (!seat || seat.name.toLowerCase() !== sender.toLowerCase())
        return false;
    const msg = message.trim().toLowerCase();
    if (['hit', 'h', 'stand', 's', 'stay'].indexOf(msg) === -1)
        return false;
    if (gs.actionTimer)
        clearTimeout(gs.actionTimer);
    if (msg === 'hit' || msg === 'h') {
...
function bjGroupCheckEliminations() {
    ...
    if (eliminated.length > 0) {
        const names = eliminated.map(function (s) {
            return s.name;
        }).join(', ');
...
function bjGroupEndByLimit() {
    ...
    if (!gs)
        return;
    const alive = gs.seats.filter(function (s) {
        return !s.eliminated;
    });
...
function showBjStatus(sender, isWhisper) {
    const session = state_1.default.activeGames.get(sender.toLowerCase());
    if (session && session.gameType === 'blackjack') {
...
function bjGroupCleanup() {
    if (state_1.default.bjGroupSession) {
        if (state_1.default.bjGroupSession.actionTimer)
            clearTimeout(state_1.default.bjGroupSession.actionTimer);
        if (state_1.default.bjGroupSession.lobbyTimer)
            clearTimeout(state_1.default.bjGroupSession.lobbyTimer);
        state_1.default.bjGroupSession = null;
    }
}
