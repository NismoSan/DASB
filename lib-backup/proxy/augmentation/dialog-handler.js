...
(function (DialogType) {
    ...
    DialogType[DialogType["Menu"] = 2] = "Menu";
    DialogType[DialogType["TextInput"] = 4] = "TextInput";
    DialogType[DialogType["Speak"] = 5] = "Speak";
    DialogType[DialogType["CloseDialog"] = 10] = "CloseDialog";
...
class DialogHandler {
    proxy;
    activeDialogs; // sessionId -> active dialog
    constructor(proxy) {
        this.proxy = proxy;
     ...
     */
    onDialogChoice(session, npc, stepId) {
        const active = this.activeDialogs.get(session.id);
        if (!active || active.npcSerial !== npc.serial)
            return;
        const config = npc.dialog;
     ...
     */
    _sendStep(session, npc, stepIndex) {
        const config = npc.dialog;
     ...
     */
    sendDialog(session, opts) {
        const pkt = new packet_1.default(0x30);
     ...
     */
    sendDialogMenu(session, opts) {
        const pkt = new packet_1.default(0x2F);
     ...
     */
    sendBoard(session, opts) {
        const pkt = new packet_1.default(0x31);
