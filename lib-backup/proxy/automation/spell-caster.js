...
class SpellCaster {
    proxy;
    session;
    spells = new Map(); // slot -> SpellInfo
    skills = new Map(); // slot -> SkillInfo
    ...
    onAddSpell(body) {
        if (body.length < 5)
            return;
        const pkt = new packet_1.default(0x17);
        pkt.body = [...body];
        pkt.position = 0;
    ...
    onAddSkill(body) {
        if (body.length < 4)
            return;
        const pkt = new packet_1.default(0x2C);
        pkt.body = [...body];
        pkt.position = 0;
        const slot = pkt.readByte();
        const icon = pkt.readUInt16();
    ...
    findSpell(name) {
        ...
        for (const spell of this.spells.values()) {
            if (spell.name.toLowerCase().includes(lower))
                return spell;
        }
        return undefined;
    }
    ...
    findSkill(name) {
        ...
        for (const skill of this.skills.values()) {
            if (skill.name.toLowerCase() === lower)
                return skill;
        }
        for (const skill of this.skills.values()) {
     ...
     */
    castSpellBySlot(slot, targetSerial) {
        const spell = this.spells.get(slot);
        // Begin chant
        if (spell && spell.castLines > 0) {
            const beginPkt = new packet_1.default(0x4D);
            ...
            for (let i = 0; i < spell.castLines; i++) {
                const chantPkt = new packet_1.default(0x4E);
                chantPkt.writeString8('');
                this.proxy.sendToServer(this.session, chantPkt);
            }
        }
        // Cast spell
        const castPkt = new packet_1.default(0x0F);
     ...
     */
    useSkillBySlot(slot) {
        const pkt = new packet_1.default(0x3E);
        pkt.writeByte(slot);
...
//# sourceMappingURL=spell-caster.js.map