...
class LootEngine {
    ...
    constructor(proxy, session, registry, config) {
        ...
        this.config = { ...exports.DEFAULT_LOOT_CONFIG, ...config };
        this.humanizer = new humanizer_1.default({ lootDelay: this.config.lootDelay });
    }
    /**
    ...
    async tryLoot() {
        ...
        if (Date.now() < this.pickupCooldown)
            return false;
        // Mob check
        if (this.config.onlyWhenNotMobbed && this.isMobbed?.())
            return false;
        // Inventory full check (60 slots)
        if (this.session.playerState.inventory.size >= 60)
            return false;
    ...
    isOtherPlayerNear(item) {
        ...
        for (const entity of entities.values()) {
            if (entity.entityType !== 'player')
                continue;
            if (entity.serial === this.session.playerState.serial)
                continue;
            const dist = Math.abs(entity.x - item.x) + Math.abs(entity.y - item.y);
            if (dist <= this.config.antiStealRadius)
                return true;
        }
        return false;
    }
    ...
    async walkToItem(item) {
        ...
        for (let step = 0; step < maxSteps; step++) {
            ...
            const items = this.registry.getGroundItems(this.session.id);
            if (!items.find(i => i.serial === item.serial))
                break;
        }
    }
    // ─── Pickup ─────────────────────────────────────────────
...
exports.default = LootEngine;
//# sourceMappingURL=loot-engine.js.map