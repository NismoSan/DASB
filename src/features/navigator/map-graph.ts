...
interface RouteNode {
  ...
  x: number;
  y: number;
  g: number;       // actual cost so far (total walking steps)
...
export default class MapGraph {
  private exits: Map<number, MapExit[]>;
  private nodes: Map<number, MapNode>;
  private filePath: string;

  constructor(filePath: string = './data/map-exits.json') {
    this.exits = new Map();
    this.nodes = new Map();
  ...
  addExit(exit: MapExit): void {
    ...
    const dupe = existing.find(e =>
      e.fromX === exit.fromX && e.fromY === exit.fromY &&
  ...
  recordTransition(fromMapId: number, fromX: number, fromY: number,
                   toMapId: number, toX: number, toY: number): void {
    const existing = this.exits.get(fromMapId) || [];
    const match = existing.find(e =>
      e.fromX === fromX && e.fromY === fromY && e.toMapId === toMapId
    );
  ...
  updateExit(fromMapId: number, fromX: number, fromY: number,
             actualToMapId: number, actualToX: number, actualToY: number): void {
   ...
   */
  findRoute(fromMapId: number, toMapId: number,
            startX: number = 0, startY: number = 0,
            targetX: number = 0, targetY: number = 0): MapExit[] | null {
    ...
    while (open.size > 0) {
      ...
      for (const exit of exits) {
        ...
        if (prevG !== undefined && totalCost >= prevG) continue;
  ...
  addWorldMapNodes(fieldName: string, nodes: { name: string; checksum: number; mapId: number; mapX: number; mapY: number }[]): void {
  ...
  getReachableMapIds(): number[] {
    ...
    this.exits.forEach((exits, fromId) => {
      ids.add(fromId);
      for (const e of exits) ids.add(e.toMapId);
    });
    return Array.from(ids).sort((a, b) => a - b);
  }

  private get nodesFilePath(): string {
  ...
  async save(): Promise<void> {
    ...
    if (nodeData.length > 0) {
      await fs.promises.writeFile(this.nodesFilePath, JSON.stringify(nodeData, null, 2));
    }
  }
  ...
  async load(): Promise<void> {
    try {
      ...
      const data: MapExit[] = JSON.parse(raw);
    ...
    try {
      const raw = await fs.promises.readFile(this.nodesFilePath, 'utf-8');
      const nodes: MapNode[] = JSON.parse(raw);
      for (const node of nodes) {
