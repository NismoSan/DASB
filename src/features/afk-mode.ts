...
const SKILL_META = {
    ...
    // ── Shadow realm skills ────────────────────────────────────────
    'assail': { bodyAnimation: BodyAnimation.Assail, effectAnimation: 0, cooldownMs: 800, type: 'damage', basePower: 150, sound: 1 },
    // ── Misc / shared skills ──────────────────────────────────────
...
/** Shadow skill book — skills injected by the AFK engine when real bars are cleared */
const SHADOW_SKILLS = {
    1: { name: 'assail', slot: 1 },
};
/** Shadow spell book — spells injected by the AFK engine when real bars are cleared */
const SHADOW_SPELLS = {};
function initAfkMode(proxy, augmentation, automation, config) {
    ...
    function getOtherAfkSessions(session, sameMapOnly = true) {
        const others = [];
        const currentMap = session.afkState?.afkMapNumber ?? null;
        for (const [, s] of proxy.sessions) {
            if (s === session || s.destroyed || !s.afkState?.active)
                continue;
            if (sameMapOnly && currentMap !== null && s.afkState.afkMapNumber !== currentMap)
                continue;
            others.push(s);
        }
        return others;
    ...
    function sendShowUserTo(target, source) {
        ...
        // Apply disguise (e.g. /king overcoat) if configured
        if (proxy.getPlayerDisguise && source.characterName && pkt.body.length >= 42) {
            const d = proxy.getPlayerDisguise(source.characterName);
            if (d && d.enabled) {
                const sprite = d.overcoatSprite != null ? d.overcoatSprite : 0;
                const color = d.overcoatColor != null ? d.overcoatColor : 0;
                if (sprite > 0) {
                    pkt.body[33] = (sprite >> 8) & 0xFF;
                    pkt.body[34] = sprite & 0xFF;
                    pkt.body[35] = color;
                }
            }
        }
    ...
    function sendEntityWalkTo(target, serial, prevX, prevY, direction) {
        ...
        pkt.writeByte(0); // trailing padding per Chaos-Server CreatureWalkConverter
        proxy.sendToClient(target, pkt);
    }
    function sendAnimateEntity(target, entityId, animation, speed, sound) {
        ...
        pkt.writeUInt16(speed); // animationSpeed as UInt16 per Chaos-Server BodyAnimationConverter
    ...
    function sendStatsUpdate(session) {
        ...
        if (!afk)
            return;
        // Send Vitality update (currentHp/currentMp) with flag 0x10
    ...
    function applyDamage(target, damage, sound) {
        ...
        const maxHpDmg = target.afkState.shadowMaxHp || target.playerState.maxHp;
        const hpPercent = (target.afkState.shadowHp / maxHpDmg) * 100;
    ...
    function applyHealing(target, amount, sound) {
        if (!target.afkState)
            return;
        const maxHpHeal = target.afkState.shadowMaxHp || target.playerState.maxHp;
        target.afkState.shadowHp = Math.min(maxHpHeal, target.afkState.shadowHp + amount);
        const hpPercent = (target.afkState.shadowHp / maxHpHeal) * 100;
        broadcastHpBar(target.playerState.serial, hpPercent, sound);
        sendStatsUpdate(target);
    }
    // ── Visibility system ───────────────────────────────────────
    ...
    let cachedMapNodes = null;
    function getMapNodeDimensions(mapId) {
        if (cachedMapNodes === null) {
            try {
                const fs = require('fs');
                const path = require('path');
                const nodesPath = path.resolve(__dirname, '../../data/map-nodes.json');
                cachedMapNodes = JSON.parse(fs.readFileSync(nodesPath, 'utf-8'));
            }
            catch (_e) {
                cachedMapNodes = [];
            }
        }
        const node = cachedMapNodes.find((entry) => entry.mapId === mapId);
        if (node && node.width > 0 && node.height > 0) {
            return { width: node.width, height: node.height };
        }
        return null;
    }
    function getShadowMapLabel(mapId) {
        return mapId === config.afkMapNumber ? 'Shadow Realm' : `Shadow Map ${mapId}`;
    }
    function getShadowMapData(mapId) {
        const mapInfo = proxy.getMapFileInfo(mapId);
        if (!mapInfo) {
            console.log(`[AfkMode] ERROR: Could not load map file for map ${mapId}`);
            return null;
        }
        let width = mapId === config.afkMapNumber ? (config.mapWidth || 0) : (mapInfo.width || 0);
        let height = mapId === config.afkMapNumber ? (config.mapHeight || 0) : (mapInfo.height || 0);
        if (!width || !height) {
            const dims = getMapNodeDimensions(mapId);
            if (dims) {
                width = dims.width;
                height = dims.height;
            }
                ...
                width = side;
                height = side;
                console.log(`[AfkMode] Auto-detected map ${mapId} dimensions: ${width}x${height} (${totalTiles} tiles)`);
            }
        }
        mapInfo.width = width;
        mapInfo.height = height;
        return { width, height, data: mapInfo.data, checksum: mapInfo.checksum };
    }
    function sendShadowMap(session, mapId, mapData, targetX, targetY, label) {
        const collision = automation.getCollision();
        collision.buildFromMapFile(mapId, mapData.width, mapData.height);
        const mapInfoPacket = new packet_1.default(0x15);
        mapInfoPacket.writeUInt16(mapId);
        ...
        const invalidChecksum = mapData.checksum ^ 0xFFFF;
        mapInfoPacket.writeUInt16(invalidChecksum);
        mapInfoPacket.writeString8(label);
        ...
        posPacket.writeUInt16(targetX);
        posPacket.writeUInt16(targetY);
        ...
        if (session.lastSelfShowUser) {
            ...
            showUserPacket.body[0] = (targetX >> 8) & 0xFF;
            showUserPacket.body[1] = targetX & 0xFF;
            showUserPacket.body[2] = (targetY >> 8) & 0xFF;
            showUserPacket.body[3] = targetY & 0xFF;
            if (proxy.getPlayerDisguise && session.characterName && showUserPacket.body.length >= 42) {
                const d = proxy.getPlayerDisguise(session.characterName);
                if (d && d.enabled) {
                    const sprite = d.overcoatSprite != null ? d.overcoatSprite : 0;
                    const color = d.overcoatColor != null ? d.overcoatColor : 0;
                    if (sprite > 0) {
                        showUserPacket.body[33] = (sprite >> 8) & 0xFF;
                        showUserPacket.body[34] = sprite & 0xFF;
                        showUserPacket.body[35] = color;
                    }
                }
            }
    ...
    function enterAfkMode(session) {
        const mapData = getShadowMapData(config.afkMapNumber);
        if (!mapData) {
        ...
        // Start MP regeneration timer (uses shadowMaxMp set by AfkEngine)
        session.afkState.mpRegenTimer = setInterval(() => {
            if (!session.afkState?.active)
                return;
            const maxMp = session.afkState.shadowMaxMp || session.playerState.maxMp;
            const regenAmount = Math.floor(maxMp * MP_REGEN_PERCENT);
            if (regenAmount > 0 && session.afkState.shadowMp < maxMp) {
                session.afkState.shadowMp = Math.min(maxMp, session.afkState.shadowMp + regenAmount);
                sendStatsUpdate(session);
            }
        }, MP_REGEN_INTERVAL);
        sendShadowMap(session, config.afkMapNumber, mapData, config.spawnX, config.spawnY, getShadowMapLabel(config.afkMapNumber));
        ...
        proxy.emit('afk:entered', session);
    }
    function teleportWithinAfkWorld(session, targetMapId, targetX, targetY, label) {
        const state = session.afkState;
        if (!state?.active)
            return;
        const mapData = getShadowMapData(targetMapId);
        if (!mapData) {
            chat.systemMessage(session, `AFK travel failed: could not load map ${targetMapId}.`);
            return;
        }
        if (targetX < 0 || targetX >= mapData.width || targetY < 0 || targetY >= mapData.height) {
            chat.systemMessage(session, `AFK travel failed: (${targetX},${targetY}) is outside map ${targetMapId}.`);
            return;
        }
        const oldMapId = state.afkMapNumber;
        removeFromAllViewports(session);
        const changingPacket = new packet_1.default(0x67);
        changingPacket.writeByte(0x00);
        changingPacket.writeUInt32(0);
        proxy.sendToClient(session, changingPacket);
        state.shadowX = targetX;
        state.shadowY = targetY;
        state.afkMapNumber = targetMapId;
        state.afkMapWidth = mapData.width;
        state.afkMapHeight = mapData.height;
        const destinationLabel = label || getShadowMapLabel(targetMapId);
        sendShadowMap(session, targetMapId, mapData, targetX, targetY, destinationLabel);
        sendStatsUpdate(session);
        augmentation.npcs.onPlayerMapChange(session);
        refreshViewport(session);
        for (const other of getOtherAfkSessions(session)) {
            if (!other.afkState)
                continue;
            if (inRange(other.afkState.shadowX, other.afkState.shadowY, targetX, targetY)) {
                sendShowUserTo(other, session);
                getVisibleSet(other).add(session.playerState.serial);
            }
        }
        console.log(`[AfkMode] ${session.characterName} moved through AFK world to map ${targetMapId} (${targetX},${targetY})`);
        chat.systemMessage(session, `AFK travel: ${destinationLabel} (map ${targetMapId}).`);
        proxy.emit('afk:mapChanged', session, oldMapId, targetMapId);
    }
    // ── Exit AFK mode ─────────────────────────────────────────────
    function exitAfkMode(session) {
        if (!session.afkState)
            return;
        proxy.emit('afk:exited', session);
    ...
    // ── Handle F5 refresh while in AFK mode ─────────────────────
    proxy.on('afk:refresh', (session) => {
        const state = session.afkState;
        if (!state?.active)
            return;
        const mapId = state.afkMapNumber;
        const mapData = getShadowMapData(mapId);
        if (!mapData) {
            console.log(`[AfkMode] Refresh failed for ${session.characterName}: could not load map ${mapId}`);
            return;
        }
        console.log(`[AfkMode] ${session.characterName} refreshing AFK world (map ${mapId} @ ${state.shadowX},${state.shadowY})`);
        sendShadowMap(session, mapId, mapData, state.shadowX, state.shadowY, getShadowMapLabel(mapId));
        sendStatsUpdate(session);
        augmentation.npcs.onPlayerMapChange(session);
        refreshViewport(session);
        for (const other of getOtherAfkSessions(session)) {
            if (!other.afkState)
                continue;
            if (inRange(other.afkState.shadowX, other.afkState.shadowY, state.shadowX, state.shadowY)) {
                sendShowUserTo(other, session);
                getVisibleSet(other).add(session.playerState.serial);
            }
        }
        proxy.emit('afk:refreshed', session);
    });
    ...
    proxy.on('afk:walk', (session, direction) => {
        ...
        walkResp.writeUInt16(11); // required trailing data per Chaos-Server
        walkResp.writeUInt16(11);
        walkResp.writeByte(1);
        proxy.sendToClient(session, walkResp);
        // Update shadow position and direction
        state.shadowX = newX;
        state.shadowY = newY;
        if (session.lastSelfShowUser) {
            session.lastSelfShowUser[4] = direction;
        }
    ...
    proxy.on('afk:castSpell', (session, slot, body) => {
        ...
        if (now - state.lastSpellTime < SPELL_THROTTLE)
            return;
        // Look up spell info — try real caster first, then shadow spells
        const auto = automation.getSession(session.id);
        const spell = auto?.caster.spells.get(slot);
        const shadowSpell = SHADOW_SPELLS[slot];
        const spellName = spell?.name?.toLowerCase() ?? shadowSpell?.name ?? '';
        const spellType = spell?.spellType ?? shadowSpell?.spellType ?? SpellTargetType.NoTarget;
        ...
        if (meta.type === 'damage' && meta.basePower > 0 && targetSerial) {
            ...
            if (target && target.afkState?.active) {
                ...
                // Try monster targeting via AfkEngine
                proxy.emit('afk:castSpell:monster', session, targetSerial, meta, spellName);
            }
        }
    ...
    proxy.on('afk:useSkill', (session, slot) => {
        ...
        if (now - state.lastSkillTime < SKILL_THROTTLE)
            return;
        // Look up skill info — try real caster first, then shadow skills
        const auto = automation.getSession(session.id);
        const skill = auto?.caster.skills.get(slot);
        const shadow = SHADOW_SKILLS[slot];
        const skillName = skill?.name?.toLowerCase() ?? shadow?.name ?? '';
        ...
        // Broadcast skill effect (0x29 ShowEffect) — skip if no effect
        if (meta.effectAnimation && meta.effectAnimation > 0) {
        ...
        if (meta.type === 'damage' && meta.basePower > 0) {
            ...
            if (target && target.afkState?.active) {
                ...
                // Try monster targeting via AfkEngine
                proxy.emit('afk:useSkill:monster', session, meta);
            }
        }
    });
    // ── Assail simulation ───────────────────────────────────────
    proxy.on('afk:assail', (session) => {
        ...
        // Apply small melee damage to target ahead (player or monster)
        const target = findTargetAhead(session);
        if (target && target.afkState?.active) {
            ...
            // Try monster targeting via AfkEngine
            proxy.emit('afk:assail:monster', session);
        }
    });
    ...
    proxy.on('afk:teleportToMap', (session, targetMapId, targetX, targetY, label) => {
        teleportWithinAfkWorld(session, targetMapId, targetX, targetY, label);
    });
