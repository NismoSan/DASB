...
const DA_PATH: string = process.env.DA_PATH || 'C:/Program Files (x86)/KRU/Dark Ages';
const CANVAS_W = 111;
const CANVAS_H = 85;
...
interface RenderLayer {
  ...
  ox?: number;
  oy?: number;
  fixBlueIndices?: boolean;
  noDyeSkip?: boolean;
  skinColor?: number;
}

function readArchive(datPath: string): Archive {
  // Only read the header/index from the archive file, not the full contents.
  // Entry data is read on-demand via getEntryData() to save memory.
  ...
  // Read the index: count * 17 bytes (4 bytes offset + 13 bytes name per entry)
  const indexSize = count * 17;
  const indexBuf = Buffer.alloc(indexSize);
  fs.readSync(fd, indexBuf, 0, indexSize, 4);
  fs.closeSync(fd);

  const entries: ArchiveEntry[] = [];
  let pos = 0;
  for (let i = 0; i < count; i++) {
    const offset = indexBuf.readUInt32LE(pos);
    pos += 4;
    const nameBytes = indexBuf.slice(pos, pos + 13);
    pos += 13;
  ...
  if (entries.length > 0) {
    entries[entries.length - 1].size = fileSize - entries[entries.length - 1].offset;
  }
  // Build name lookup map for fast access
  const nameMap: Record<string, ArchiveEntry> = {};
  for (const entry of entries) {
    nameMap[entry.name] = entry;
    nameMap[entry.name.toLowerCase()] = entry;
  }
  return { filePath: datPath, entries, count, nameMap, fileSize };
}

function getEntryData(archive: Archive, entryName: string): Buffer | null {
...
function readPalette(data: Buffer): Palette | null {
...
function readPalTable(data: Buffer): PalTable {
  if (!data) return { overrides: {}, maleOverrides: {}, femaleOverrides: {}, entries: {} };
  const text = data.toString('ascii');
  const lines = text.split(/\r?\n/).filter((l: string) => l.trim());
  const overrides: Record<number, number> = {};
  const maleOverrides: Record<number, number> = {};
  const femaleOverrides: Record<number, number> = {};
  const entries: Record<number, number> = {}; // range entries expanded to individual IDs (matches DALib)
...
function findPaletteIdx(table: PalTable, spriteId: number, isFemale: boolean): number {
  if (!table) return 0;
...
function hasPalTableEntry(table: PalTable | undefined, spriteId: number, isFemale: boolean): boolean {
...
function readColorTable(data: Buffer | null): Map<number, PaletteColor[]> | null {
  if (!data) return null;
  const text = data.toString('ascii');
  const lines = text.split(/\r?\n/).filter((l: string) => l.trim());
  ...
  const table = new Map<number, PaletteColor[]>();
  let i = 1;
  while (i < lines.length) {
    ...
    for (let c = 0; c < colorsPerEntry && i < lines.length; c++, i++) {
      const parts = lines[i].split(',').map((v: string) => parseInt(v, 10));
...
export class SpriteRenderer {
  ...
  init(): boolean {
    ...
    try {
      ...
      for (const file of khanFiles) {
        const filePath = path.join(this.daPath, file);
      ...
      const palPath = path.join(this.daPath, 'khanpal.dat');
      ...
      const legendPath = path.join(this.daPath, 'legend.dat');
  ...
  findEpf(fileName: string): Epf | null {
    ...
    for (const arch of this.khanArchives) {
      ...
      if (data) {
        ...
        // Limit EPF cache to 200 entries to prevent unbounded memory growth
        const epfKeys = Object.keys(this.epfCache);
        if (epfKeys.length > 200) {
          for (let i = 0; i < 50; i++) delete this.epfCache[epfKeys[i]];
        }
  ...
  loadPalette(letter: string, idx: number): Palette | null {
    ...
    // Limit palette cache to prevent unbounded growth
    const palKeys = Object.keys(this.paletteCache);
    if (palKeys.length > 500) {
      for (let i = 0; i < 100; i++) delete this.paletteCache[palKeys[i]];
    }
    const names = [
  ...
  applySkinColor(basePalette: Palette, skinColor: number): Palette {
    ...
    const cacheKey = 'skin:' + skinColor + ':' + (basePalette as any)._id;
    if ((basePalette as any)._id && this.paletteCache[cacheKey]) return this.paletteCache[cacheKey];

    const merged: PaletteColor[] = new Array(basePalette.length);
    for (let i = 0; i < basePalette.length; i++) {
      merged[i] = basePalette[i];
    }
    // Overlay skin tone ramp from palm (16-31)
    for (let i = 16; i <= 31; i++) {
      if (skinPal[i]) merged[i] = skinPal[i];
    }
    ...
    if (baseSkinPal && skinColor !== 0) {
      ...
      for (let i = 17; i <= 30; i++) {
        ...
        if (p0 && ps && p0.r > 0 && p0.g > 0 && p0.b > 0) {
          rr += ps.r / p0.r; rg += ps.g / p0.g; rb += ps.b / p0.b;
          cnt++;
        }
      }
      if (cnt > 0) {
    ...
    // Tag for caching
    if (!(basePalette as any)._id) (basePalette as any)._id = Math.random().toString(36).slice(2);
  ...
  fixPalmBlueIndices(palmPalette: Palette, skinColor: number): Palette {
    ...
    const merged: PaletteColor[] = new Array(palmPalette.length);
    ...
    const skinIndexMap: Record<number, number> = {
      61: 22, 62: 24, 63: 26,
    ...
    if (palbPalette) {
      for (let i = 10; i <= 15; i++) {
        if (palbPalette[i]) merged[i] = palbPalette[i];
      }
    }
  ...
  renderCharacter(appearance: any): Buffer | null {
    ...
    if (Object.keys(this.renderCache).length > 100) {
      const keys = Object.keys(this.renderCache);
      for (let i = 0; i < 50; i++) delete this.renderCache[keys[i]];
    }
  ...
  getStats(): { khanArchives: number; palTables: number; cachedPalettes: number; cachedEpfs: number; cachedRenders: number } | null {
...
let rendererInstance: SpriteRenderer | null = null;
