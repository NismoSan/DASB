...
const PANEL_DIR = path.join(APP_ROOT, 'panel');
const DATA_DIR = path.join(APP_ROOT, 'data');
const Client = require('./core/client').default;
const Packet = require('./core/packet').default;
var aeIngest = require('./features/ae-ingest');
var discord = require('./features/discord');
var chatGames = require('./games');
var tradeSessions = require('./features/trade-sessions');
var aiChat = require('./features/ai-chat');
var iconv = require('iconv-lite');
var db = require('./features/database');
var packetStore = require('./features/packet-store');

// Extracted modules
var auth = require('./features/auth');
var configManager = require('./features/config-manager');
var opcodes = require('./core/opcodes');
var playerTracker = require('./features/player-tracker');
var scheduledMessages = require('./features/scheduled-messages');
var { getSpriteRenderer } = require('./features/sprite-renderer');
var fs = require('fs');
var { Navigator } = require('./features/navigator');
var lottery = require('./features/lottery');
var sense = require('./features/sense');
var itemTrade = require('./features/item-trade');
var npcLeak = require('./features/npc-leak');
var slotMachine = require('./features/slot-machine');
var auctionHouse = require('./features/auction-house');
var monsterKeeper = require('./features/monster-capture/keeper-npc');
var fishing = require('./features/fishing');
var eggCollector = require('./features/egg-collector');
var eggBanker = require('./features/egg-banker');
var { createProxySystem } = require('./proxy/index');

// ── Proxy System ────────────────────────────────────────────────
var proxySystem: any = null;
var hotReloader: any = null;

// ── Hot-Reload Feature Registry ─────────────────────────────────
var featureRegistry: Map<string, any> = new Map();
...
var captureOpcodes = new Set<number>();
var capturedPackets: any[] = [];
var MAX_CAPTURED = 200;

// ── Custom Legend Marks System ──────────────────────────────────
var CUSTOM_LEGENDS_FILE = path.join(DATA_DIR, 'custom-legends.json');
var customLegendMarks: any[] = []; // Array of { id, icon, color, key, text, issuedTo: [playerName, ...] }
...
var PLAYER_NAMETAG_STYLES_FILE = path.join(DATA_DIR, 'player-nametag-styles.json');
var playerNameTagStyles: Record<string, any> = {}; // { playerName: styleByte }
...
var DISGUISE_STATE_FILE = path.join(DATA_DIR, 'disguise-state.json');
var playerDisguises: Record<string, any> = {}; // { playerName: { enabled, title, displayClass, guildRank, guild, overcoatSprite, overcoatColor } }
...
var PROXY_FIRSTLOGIN_FILE = path.join(DATA_DIR, 'proxy-firstlogins.json');
var proxyFirstLogins: Record<string, any> = {}; // { playerName: "2026-03-15T..." }
...
var pendingAuctioneerSerials: any[] = [];
...
var recentSentWhispers: Map<string, number> = new Map(); // key: "target|message" → expiry timestamp
...
var bots: Map<any, any> = new Map();

function createBotState(botConfig): any {
  return {
    id: botConfig.id,
    status: 'disconnected',
    connectedAt: null,
...
function isEggCollectorBot(botId) {
  var bot = bots.get(botId);
  return bot && bot.config.role === 'egg_collector';
}

function getEggCollectorBot(): any {
  var found: any = null;
  bots.forEach(function (bot) {
    if (bot.config.role === 'egg_collector' && bot.client && bot.state.status === 'logged_in') {
      found = bot;
    }
  });
  return found;
...
function isEggBankerBot(botId) {
  var bot = bots.get(botId);
  return bot && bot.config.role === 'egg_banker';
}

function getEggBankerBot(): any {
  var found: any = null;
  bots.forEach(function (bot) {
    if (bot.config.role === 'egg_banker' && bot.client && bot.state.status === 'logged_in') {
      found = bot;
    }
  });
  return found;
...
function eggCollectorSendSay(message) {
  var bot = getEggCollectorBot();
  if (bot && bot.client && bot.state.status === 'logged_in') {
    var p = new Packet(0x0E);
    p.writeByte(0x00);
    p.writeString8(message.substring(0, 64));
    bot.client.send(p);
...
function eggCollectorSendWhisper(target, message) {
  var bot = getEggCollectorBot();
  if (bot && bot.client && bot.state.status === 'logged_in') {
    recordSentWhisper(target, message.substring(0, 64));
    var p = new Packet(0x19);
...
function eggCollectorSendPacket(packet) {
  var bot = getEggCollectorBot();
  if (bot && bot.client && bot.state.status === 'logged_in') {
    bot.client.send(packet);
  }
}

function eggBankerSendPacket(packet) {
  var bot = getEggBankerBot();
  if (bot && bot.client && bot.state.status === 'logged_in') {
    bot.client.send(packet);
  }
}

function eggBankerSendWhisper(target, message) {
  var bot = getEggBankerBot();
  if (bot && bot.client && bot.state.status === 'logged_in') {
    recordSentWhisper(target, message.substring(0, 64));
    var p = new Packet(0x19);
...
// ── Login Rate Limiting ─────────────────────────────────────────
var loginAttempts: Record<string, number> = {};
setInterval(function () { loginAttempts = {}; }, 60000);

// ── Auth Routes ──────────────────────────────────────────────────

app.get('/login', function (req, res) {
  res.sendFile(path.join(PANEL_DIR, 'login.html'));
});
...
app.get('/health', function (_req, res) {
  db.pool.query('SELECT 1').then(function () {
    ...
    res.json({
      ...
      players: Object.keys(playerTracker.getPlayerDB()).length,
      bots: bots.size,
      chatHistory: chatHistory.length,
      capturedPackets: capturedPackets.length
    });
  }).catch(function () {
...
app.get('/chat', function (req, res) {
  res.sendFile(path.join(PANEL_DIR, 'chat.html'));
});
app.use(express.static(PANEL_DIR, { etag: true, maxAge: 86400000 }));
...
var SPRITE_OVERRIDES_PATH = path.join(DATA_DIR, 'sprite-overrides.json');
...
app.get('/api/sprite/head-preview/:headId.png', function (req, res) {
  ...
  var baseAppearance: any = null;
  if (req.query.base) {
    try { baseAppearance = JSON.parse(req.query.base); } catch (e: any) {
...
app.post('/api/lottery/sync', function (req, res) {
  ...
  var uniquePlayers: Record<string, boolean> = {};
...
function attachGameHandlers(c, botId) {
        ...
        if (name && serial) {
          ...
          if (name !== bot.state.username && playerTracker.canRequestProfile(name) && !(isSlotBot(botId) && slotBankingActive)) {
            try {
              ...
              c.send(profileReq);
              // console.log('[Legend] Sent profile request for ' + name + ' (serial ' + serial + ')');
            } catch (e2: any) { console.log('[Legend] Failed to send profile request for ' + name + ': ' + e2.message); }
          }
    ...
    for (var nli = 0; nli < npcLeakOpcodes.length; nli++) {
      (function (op) {
        c.events.on(op, function (packet) {
          ...
          } catch (e: any) { /* don't break normal flow */ }
          packet.position = saved;
        });
      })(npcLeakOpcodes[nli]);
    }
  ...
  // ── Egg Collector Bot Exchange Handlers ─────────────────────────
  if (isEggCollectorBot(botId)) {
    c.events.on(0x42, function (packet) {
      eggCollector.handleExchangeMessage(packet);
    });

    c.events.on(0x4B, function (packet) {
      eggCollector.handleExchangeSlot(packet);
    });

    c.events.on(0x37, function (packet) {
      eggCollector.handleAddItem(packet);
    });

    c.events.on(0x38, function (packet) {
      eggCollector.handleRemoveItem(packet);
    });

    // Whisper handling for egg collector bot
    c.events.on(0x0A, function (packet) {
      var saved = packet.position;
      var channelByte = packet.readByte();
      var messageRaw = '';
      ...
      var whisperMatch = messageRaw.match(/^"?([A-Za-z][^"]*)"\s*([\s\S]*)$/);
      if (whisperMatch && !isSentWhisperEcho(whisperMatch[1], whisperMatch[2])) {
        eggCollector.handleWhisper(whisperMatch[1], whisperMatch[2]);
      }
    });
  }

  // ── Egg Banker Bot Exchange Handlers ──────────────────────────────
  if (isEggBankerBot(botId)) {
    c.events.on(0x42, function (packet) {
      eggBanker.handleExchangeMessage(packet);
    });

    c.events.on(0x4B, function (packet) {
      eggBanker.handleExchangeSlot(packet);
    });

    c.events.on(0x37, function (packet) {
      eggBanker.handleAddItem(packet);
    });

    c.events.on(0x38, function (packet) {
      eggBanker.handleRemoveItem(packet);
    });
  }
...
function stopBot(botId) {
  ...
  if (!bot) return;
  if (bot.navigator) {
    try { bot.navigator.stop(); } catch (e: any) { /* ignore */ }
    bot.navigator = null;
  }
  if (bot.client) {
    try { bot.client.events.removeAllListeners(); } catch (e: any) { /* ignore */ }
    try { bot.client.stop(); } catch (e: any) { /* ignore */ }
    bot.client = null;
  }
  bot.state.status = 'disconnected';
  bot.state.connectedAt = null;
...
function forceResetBot(botId) {
  ...
  if (!bot) return;
  if (bot.client) {
    try { bot.client._cancelAutoReconnect(); } catch (e: any) { /* ignore */ }
    try { bot.client._stopped = true; } catch (e: any) { /* ignore */ }
    try { bot.client.events.removeAllListeners(); } catch (e: any) { /* ignore */ }
    try { bot.client.disconnect(); } catch (e: any) { /* ignore */ }
    try {
...
io.on('connection', function (socket) {
  ...
  if (proxySystem) {
    ...
    socket.on('proxy:capture:start', function (data) {
      captureEnabled = true;
      if (proxySystem && proxySystem.inspector) proxySystem.inspector.captureBody = true;
    ...
    socket.on('proxy:capture:stop', function () {
      ...
      if (proxySystem && proxySystem.inspector) proxySystem.inspector.captureBody = false;
  ...
  socket.on('monsters:search', function (data) {
    if (!data || !data.playerName) return;
    var monsterDb = require('./features/monster-capture/monster-db');
  ...
  socket.on('fishing:derbyState', function () {
    try {
      require('./features/fishing/derby').getFishingDerbyPanelState().then(function (state) {
  ...
  socket.on('league:getState', function () {
    try {
      require('./features/monster-capture/league').getLeaguePanelState().then(function (state) {
  ...
  socket.on('monsters:getData', function () {
    try {
      var speciesData = require('./features/monster-capture/species-data');
  ...
  socket.on('nav:getMapList', function () {
    var nodeMap: Record<string, any> = {};
    var idSet: Record<string, boolean> = {};

    // 1. Load from persisted files first (always available even with no bots connected)
    try {
      var nodesRaw = fs.readFileSync(path.join(DATA_DIR, 'map-nodes.json'), 'utf-8');
      var fileNodes = JSON.parse(nodesRaw);
      fileNodes.forEach(function (n) { nodeMap[n.mapId] = n; });
    } catch (e: any) { /* no file yet */ }
    try {
      var exitsRaw = fs.readFileSync(path.join(DATA_DIR, 'map-exits.json'), 'utf-8');
...
eggCollector.init({
  sendPacket: eggCollectorSendPacket,
  sendWhisper: eggCollectorSendWhisper,
  sendSay: eggCollectorSendSay,
  io: io,
  getBotSerial: function () {
    var bot = getEggCollectorBot();
    return bot ? (bot.state.serial || 0) : 0;
  },
  getEntityName: function (serial) {
    var bot = getEggCollectorBot();
    if (bot && bot.entityNames) {
      return bot.entityNames[serial];
    }
    return undefined;
  },
  getSerialByName: function (name) {
    var bot = getEggCollectorBot();
    if (bot && bot.entityNames) {
      for (var s in bot.entityNames) {
        if (bot.entityNames[s].toLowerCase() === name.toLowerCase()) return parseInt(s);
      }
    }
    return 0;
  }
...
eggBanker.init({
  sendPacket: eggBankerSendPacket,
  sendWhisper: eggBankerSendWhisper,
  io: io,
  getBotSerial: function () {
    var bot = getEggBankerBot();
    return bot ? (bot.state.serial || 0) : 0;
...
registerFeature('playerTracker', './features/player-tracker',
  function (m) { playerTracker = m; },
  function () { return { db: db, io: io }; }
);
registerFeature('scheduledMessages', './features/scheduled-messages',
...
registerFeature('aeIngest', './features/ae-ingest',
  function (m) { aeIngest = m; },
  function () { return { aeIngest: loadConfig().aeIngest }; }
);
registerFeature('discord', './features/discord',
  function (m) { discord = m; },
  function () { return [null, db]; },
  'spread'
);
registerFeature('chatGames', './games',
...
registerFeature('tradeSessions', './features/trade-sessions',
...
registerFeature('aiChat', './features/ai-chat',
  function (m) { aiChat = m; },
  function () {
    return {
...
registerFeature('lottery', './features/lottery',
...
registerFeature('eggCollector', './features/egg-collector',
  function (m) { eggCollector = m; },
  function () {
    return {
      sendPacket: eggCollectorSendPacket, sendWhisper: eggCollectorSendWhisper, sendSay: eggCollectorSendSay, io: io,
      getBotSerial: function () { var b = getEggCollectorBot(); return b ? (b.state.serial || 0) : 0; },
      getEntityName: function (serial) { var b = getEggCollectorBot(); return b && b.entityNames ? b.entityNames[serial] : undefined; },
      getSerialByName: function (name) {
        var b = getEggCollectorBot();
        if (b && b.entityNames) {
          for (var s in b.entityNames) { if (b.entityNames[s].toLowerCase() === name.toLowerCase()) return parseInt(s); }
        }
        return 0;
      }
    };
...
registerFeature('eggBanker', './features/egg-banker',
  function (m) { eggBanker = m; },
  function () {
    return {
      sendPacket: eggBankerSendPacket, sendWhisper: eggBankerSendWhisper, io: io,
      getBotSerial: function () { var b = getEggBankerBot(); return b ? (b.state.serial || 0) : 0; },
      getEntityName: function (serial) { var b = getEggBankerBot(); return b && b.entityNames ? b.entityNames[serial] : undefined; }
    };
  }
);
registerFeature('slotMachine', './features/slot-machine',
...
registerFeature('sense', './features/sense',
  function (m) { sense = m; },
  function () { return undefined; }  // sense.init() is called per-bot, not globally
);
registerFeature('itemTrade', './features/item-trade',
...
db.init().then(function () {
  ...
  // Auto-start all enabled bots on server restart (skip in dev mode)
  if (process.env.DEV !== 'true' && config.bots && config.bots.length > 0) {
    console.log('[Startup] Auto-starting all enabled bots...');
    startAllBots();
  } else if (process.env.DEV === 'true') {
    console.log('[Startup] DEV mode — skipping bot auto-start');
  }
  ...
  function gracefulShutdown() {
    ...
    var chat = proxySystem && proxySystem.augmentation && proxySystem.augmentation.chat;
    var hasPlayers = proxySystem && proxySystem.server && proxySystem.server.sessions && proxySystem.server.sessions.size > 0;

    if (!chat || !hasPlayers) {
      console.log('\n[Shutdown] No active players. Shutting down immediately...');
      performShutdown();
      return;
    }

    console.log('\n[Shutdown] Restart signal received. Broadcasting 2-minute warning...');
    chat.systemBroadcast('Server will restart in 2 minutes.');

    var warnings = [
      { at: 60, msg: 'Server restarting in 60 seconds.' },
      { at: 30, msg: 'Server restarting in 30 seconds.' },
      { at: 15, msg: 'Server restarting in 15 seconds.' },
      { at: 10, msg: 'Server restarting in 10 seconds.' },
      { at: 5,  msg: 'Server restarting in 5 seconds.' },
      { at: 4,  msg: 'Server restarting in 4...' },
      { at: 3,  msg: 'Server restarting in 3...' },
      { at: 2,  msg: 'Server restarting in 2...' },
      { at: 1,  msg: 'Server restarting in 1...' },
    ];

    var totalSeconds = 120;
    for (var i = 0; i < warnings.length; i++) {
      (function (w) {
        setTimeout(function () { chat.systemBroadcast(w.msg); }, (totalSeconds - w.at) * 1000);
      })(warnings[i]);
    }

    setTimeout(performShutdown, totalSeconds * 1000);
  }
  ...
  server.listen(webPort, function () {
    ...
    if (proxyConfig && proxyConfig.enabled) {
      proxySystem = createProxySystem({
        ...
        realServerAddress: proxyConfig.realServerAddress || process.env.DA_SERVER_ADDRESS || '127.0.0.1',
      ...
      // ── MCP send_packet command queue polling ──────────────────
      setInterval(function () {
        if (!proxySystem) return;
        var commands = packetStore.pollPendingSendCommands();
        for (var cmd of commands) {
          // Find session by character name
          var targetSession = null;
          for (var [, sess] of proxySystem.server.sessions) {
            if (sess.characterName && sess.characterName.toLowerCase() === cmd.character.toLowerCase()) {
              targetSession = sess;
              break;
            }
          }
          if (!targetSession) {
            console.log('[MCP SendPacket] No session found for character "' + cmd.character + '" (cmd #' + cmd.id + ')');
            packetStore.markSendCommandDone(cmd.id, 'error:no-session');
            continue;
          }
          try {
            var Packet = require('./core/packet').default;
            var crcMod = require('./core/crc');
            var uint8Fn = require('./core/datatypes').uint8;
            var bodyBytes = cmd.hex_body.trim().split(/\s+/).map(function (h: string) { return parseInt(h, 16); });
            var pkt = new Packet(cmd.opcode);
            // For 0x39/0x3A to-server: apply dialog sub-encryption before standard crypto
            if (cmd.direction === 'to-server' && (cmd.opcode === 0x39 || cmd.opcode === 0x3A)) {
              var payload = bodyBytes;
              var x = Math.floor(Math.random() * 256);
              var xPrime = Math.floor(Math.random() * 256);
              var y = uint8Fn(x + 0x72);
              var z = uint8Fn(x + 0x28);
              var crc = crcMod.calculateCRC16(payload);
              var plain = [(crc >> 8) & 0xFF, crc & 0xFF].concat(payload);
              var dataLength = plain.length;
              var encrypted: number[] = [];
              for (var j = 0; j < plain.length; j++) {
                encrypted.push(uint8Fn(plain[j] ^ uint8Fn((z + j) & 0xFF)));
              }
              var lenHi = uint8Fn((dataLength >> 8) ^ y);
              var lenLo = uint8Fn((dataLength & 0xFF) ^ uint8Fn((y + 1) & 0xFF));
              var b0 = uint8Fn(xPrime + 0x2D);
              var b1 = uint8Fn(x ^ xPrime);
              pkt.body = [b0, b1, lenHi, lenLo].concat(encrypted);
              console.log('[MCP SendPacket] Applied dialog sub-encryption for 0x' + cmd.opcode.toString(16) + ' (plaintext ' + payload.length + 'B -> encrypted ' + pkt.body.length + 'B)');
            } else {
              pkt.body = bodyBytes;
            }
            if (cmd.direction === 'to-server') {
              proxySystem.server.sendToServer(targetSession, pkt);
            } else {
              proxySystem.server.sendToClient(targetSession, pkt);
            }
            console.log('[MCP SendPacket] Executed cmd #' + cmd.id + ': ' + cmd.direction + ' 0x' + cmd.opcode.toString(16).padStart(2, '0') + ' to ' + cmd.character + ' (' + bodyBytes.length + 'B)');
            packetStore.markSendCommandDone(cmd.id, 'done');
          } catch (err: any) {
            console.log('[MCP SendPacket] Error executing cmd #' + cmd.id + ': ' + (err && err.message));
            packetStore.markSendCommandDone(cmd.id, 'error:' + (err && err.message));
          }
        }
      }, 500);

      // Packet logging to panel (throttled)
      var packetLogBuffer: any[] = [];
      var packetLogTimer: any = null;
      ...
      proxySystem.inspector.onPacket = function (packet, direction, session) {
        ...
        // Persist to database for MCP tool queries (only when capture is active)
        if (captureEnabled && packet.body && packet.body.length > 0) {
          ...
          // Also persist to local SQLite for MCP server access
          packetStore.persistPacketCapture(session.id, session.characterName || '', direction, packet.opcode, opName, packet.body.length, hexForDb);
        }
      ...
      proxySystem.server.start().then(function () {
        ...
        if (monsterConfig && monsterConfig.enabled) {
          var monsterCapture = require('./features/monster-capture/index');
        ...
        var afkMode = require('./features/afk-mode');
        var afkConfig = {
          afkMapNumber: 6002,
          mapWidth: 60,
          mapHeight: 25,
        ...
        try {
          var AfkEngine = require('./features/afk').AfkEngine;
    ...
    var HotReloader = require('./features/hot-reloader').HotReloader;
