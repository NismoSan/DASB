...
export default class ProxyCollision {
    ...
    async loadSotp(filePath) {
        try {
            this.sotp = await fs.promises.readFile(filePath);
    ...
    buildFromPendingRows(mapId) {
        ...
        for (let y = 0; y < height; y++) {
            ...
            for (let x = 0; x < width; x++) {
                const offset = x * 6;
                if (offset + 5 >= tileBytes.length) break;
    ...
    buildFromMapFile(mapId, width, height) {
        if (!this.sotp) return false;

        const mapFilePath = path.join(this.mapsDir, `lod${mapId}.map`);
        let buf;
        try {
            buf = fs.readFileSync(mapFilePath);
        } catch {
            return false;
        }
