...
import { SPELL_BAR_ICONS } from './buff-tracker';
...
export interface HealConfig {
    ...
    selfBuffs: { spell: string; icon: number }[];
...
const ITEM_COOLDOWNS: Record<string, number> = {
...
export default class HealEngine {
    ...
    config: HealConfig;
    private lastItemUse: Map<string, number> = new Map();
    ...
    constructor(
        ...
        this.humanizer = new Humanizer({ reactDelay: this.config.reactDelay });
    }
     ...
     */
    async healCycle(): Promise<boolean> {
    ...
    private async tryUsePotion(priority: string[]): Promise<boolean> {
        for (const potionName of priority) {
            const slot = this.findInventoryItem(potionName);
            if (slot !== undefined) {
                if (this.isItemOnCooldown(potionName)) continue;
    ...
    private findInventoryItem(name: string): number | undefined {
    ...
    private isItemOnCooldown(name: string): boolean {
    ...
    private useItemBySlot(slot: number): void {
        const pkt = new Packet(0x1C);
        pkt.writeByte(slot);
    ...
    private async maintainSelfBuffs(): Promise<boolean> {
        let acted = false;
    ...
    private async aoCures(): Promise<boolean> {
        let acted = false;
        ...
        if (this.config.aoSuainSelf) {
            ...
            if (this.buffs.hasSpellBarEffect(SPELL_BAR_ICONS.STATUS_1) ||
                this.buffs.hasSpellBarEffect(SPELL_BAR_ICONS.STATUS_2)) {
    ...
    private async checkDionEmergency(hpPercent: number): Promise<boolean> {
        // Already have dion
        if (this.buffs.hasSelfDion()) return false;
    ...
    private getVisibleMonsterCount(): number {
