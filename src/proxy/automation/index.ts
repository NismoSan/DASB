...
export default class AutomationManager {
    ...
    constructor(proxy) {
        this.proxy = proxy;
        this.collision = new ProxyCollision();
        this.mapGraph = new MapGraph('./data/map-exits.json');
    }
    ...
    createSession(session) {
        const navigator = new ProxyNavigator(this.proxy, session, this.collision, this.mapGraph);
        const caster = new SpellCaster(this.proxy, session);
        const buffs = new BuffTracker();
        // Combat engine needs registry - use placeholder if not yet wired
        const registry = this.registry;
        const combat = new CombatEngine(this.proxy, session, registry, caster, buffs);
        const heal = new HealEngine(this.proxy, session, caster, buffs);
        const loot = new LootEngine(this.proxy, session, registry);
        const desync = new DesyncMonitor(this.proxy, session);
    ...
    destroySession(sessionId) {
        ...
        if (auto) {
            ...
            auto.desync.destroy();
            if (auto.followCancel) auto.followCancel();
    ...
    registerCommands(commands, chat) {
        const DIR_MAP = {
            up: Direction.Up, north: Direction.Up, n: Direction.Up, u: Direction.Up,
            right: Direction.Right, east: Direction.Right, e: Direction.Right, r: Direction.Right,
            down: Direction.Down, south: Direction.Down, s: Direction.Down, d: Direction.Down,
            left: Direction.Left, west: Direction.Left, w: Direction.Left, l: Direction.Left,
        };
        ...
        commands.register('buffs', (session) => {
            ...
            // Named buffs
            const named = [];
            if (auto.buffs.hasSelfAite()) named.push('Aite');
            if (auto.buffs.hasSelfFas()) named.push('Fas');
            if (auto.buffs.hasSelfDion()) named.push('Dion');
            if (auto.buffs.hasSelfCounterAttack()) named.push('Counter Attack');
            if (auto.buffs.hasSelfCradh()) named.push('Cradh (CURSED)');
            if (auto.buffs.hasSelfPoison()) named.push('Poison');
            if (auto.buffs.hasSelfHide()) named.push('Hide');
            if (auto.buffs.hasSelfDruidForm()) named.push('Druid Form');
            if (named.length > 0) {
