...
export default class CombatEngine {
    ...
    private lastSecondaryCast: number = 0;
    private lastAssailTime: number = 0;
    /** External heal check -- set by HealEngine when wired. */
    healCheck: (() => Promise<boolean>) | null = null;
    ...
    constructor(
        ...
        this.caster = caster;
        this.buffs = buffs;
        this.config = { ...DEFAULT_GRIND_CONFIG, ...config };
    ...
    private async combatLoop(): Promise<void> {
        while (this.running) {
            try {
                ...
                // Step 0: Heal check
                if (await this.doHealCheck()) continue;
    ...
    private async executeSpellChain(target: EntityInfo): Promise<void> {
        // 1. Secondary attack (Cursed Tune) with cooldown
        if (await this.trySecondaryAttack(target)) return;
        // 2. Per-monster custom config
        if (await this.tryMonsterConfig(target)) return;
        // 3. Pramh/stun-lock
        if (await this.tryPramhPhase(target)) return;
        // 4. Fas/Cradh curse phase
        if (await this.tryCursePhase(target)) return;
        // 5. Primary attack
        if (await this.tryPrimaryAttack(target)) return;
    ...
    private async trySecondaryAttack(target: EntityInfo): Promise<boolean> {
        if (!this.config.secondaryAttack) return false;
        if (!this.running) return true;

        const now = Date.now();
        if (now - this.lastSecondaryCast < this.config.secondaryCooldownMs) return false;
        ...
        if (this.castOnTarget(this.config.secondaryAttack, target.serial)) {
            ...
            if (await this.doHealCheck()) return true;
            return !this.isTargetAlive(target);
        }
        return false;
    }

    private async tryMonsterConfig(target: EntityInfo): Promise<boolean> {
    ...
    private async tryPramhPhase(target: EntityInfo): Promise<boolean> {
        if (!this.config.pramhSpell) return false;
        if (!this.running) return true;

        const spell = this.config.pramhSpell;
        // Get cluster for AoE stun
        const monsters = this.registry.getMonsters(this.session.id);
        const cluster = this.targetSelector.selectCluster(
            monsters, this.session.playerState.x, this.session.playerState.y, this.buffs
        );

        // Cast pramh on unstunned targets in cluster
        for (const entity of cluster) {
            if (!this.running) return true;
            if (this.buffs.isStunned(entity.serial) && !this.config.pramhSpam) continue;
        ...
        if (this.config.pramhOnly) return true;
    ...
    private async tryCursePhase(target: EntityInfo): Promise<boolean> {
        if (!this.running) return true;
    ...
    private async curseTarget(entity: EntityInfo): Promise<void> {
        if (!this.running) return;
        // Fas first
    ...
    private getCurseCandidates(monsters: EntityInfo[]): EntityInfo[] {
    ...
    private async tryPrimaryAttack(target: EntityInfo): Promise<boolean> {
        if (!this.config.primaryAttack) return false;
        if (!this.running) return true;
    ...
    private async doAssail(): Promise<void> {
        if (!this.running) return;
        const now = Date.now();
        if (now - this.lastAssailTime < 250) return; // min 250ms between assails
    ...
    private findTarget(): EntityInfo | null {
        const monsters = this.registry.getMonsters(this.session.id);
        return this.targetSelector.selectTarget(
            monsters, this.session.playerState.x, this.session.playerState.y, this.buffs
        );
    }

    private isTargetAlive(target: EntityInfo): boolean {
        const entity = this.registry.getEntity(this.session.id, target.serial);
        return entity !== undefined;
    }

    private distanceTo(target: EntityInfo): number {
        return Math.abs(target.x - this.session.playerState.x) +
            Math.abs(target.y - this.session.playerState.y);
    }

    private castOnTarget(spellName: string, targetSerial: number): boolean {
        return this.caster.castSpell(spellName, targetSerial);
    }

    private faceTarget(target: EntityInfo): void {
    ...
    private async walkToTarget(target: EntityInfo): Promise<void> {
    ...
    private async doHealCheck(): Promise<boolean> {
        if (this.healCheck) {
